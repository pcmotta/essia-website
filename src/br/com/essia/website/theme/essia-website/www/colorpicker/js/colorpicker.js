...
(function ($) {
	var ColorPicker = function () {
		var
			...
			fillRGBFields = function  (hsb, cal) {
				...
				$(cal).data('colorpicker').fields
					.eq(1).val(Math.ceil(rgb.r)).end()
					.eq(2).val(Math.ceil(rgb.g)).end()
					.eq(3).val(Math.ceil(rgb.b)).end();
			},
			fillHSBFields = function  (hsb, cal) {
				$(cal).data('colorpicker').fields
					.eq(4).val(Math.ceil(hsb.h)).end()
					.eq(5).val(Math.ceil(hsb.s)).end()
					.eq(6).val(Math.ceil(hsb.b)).end();
			},
			...
			HSBToHex = function (hsba) {
				return RGBToHex(HSBToRGB(hsba));
			},
		...
		return {
			...
			getId: function(){
				return this.each(function(){
					if ($(this).data('colorpickerId')) {
						return $(this).data('colorpickerId');
					}
				});
			}
		};
	...
	$.fn.extend({
		...
		ColorPickerGetId: ColorPicker.getId
	});
})(jQuery)